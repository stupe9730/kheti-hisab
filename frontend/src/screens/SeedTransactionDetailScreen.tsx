...
import { ArrowLeft, FileText, Download, TrendingUp, History, ShoppingBag, Sprout } from 'lucide-react';
...
export default function SeedTransactionDetailScreen() {
      ...
      if (entryId && farmId) {
        try {
          const [historyData, farms] = await Promise.all([
            storage.getSeedHistory(entryId),
            storage.getFarms()
          ]);
  ...
  const generatePDF = () => {
    ...
    doc.text('Seeds (Beej) Audit Report', 14, 20);
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('Seed Details:', 14, 50);
    doc.setFont('helvetica', 'normal');
    doc.text(`${original.seedName} ${original.seedCompany ? `(${original.seedCompany})` : ''} (${original.cropType}) | ${original.quantity} ${original.unit} | RS ${original.price}/unit`, 45, 50);
    ...
    drawCard(77, 'Total Paid', `RS ${latest.totalPaidAmount.toLocaleString()}`, [22, 163, 74]);
    ...
    history.forEach((entry, idx) => {
      ...
      if (isOriginal) {
        doc.setFont('helvetica', 'bold');
        doc.text('Seed Name:', 18, currentY + 20);
        doc.setFont('helvetica', 'normal');
        doc.text(`${entry.seedName} ${entry.seedCompany ? `(${entry.seedCompany})` : ''} (${entry.cropType})`, 45, currentY + 20);
    ...
    doc.save(`Seed_Audit_${farm.name}_${format(new Date(), 'yyyyMMdd')}.pdf`);
  };
    ...
    <div className="space-y-6 pb-20 p-4 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            <h2 className="text-xl font-black text-foreground tracking-tight text-left">Seed Audit Trail</h2>
            <p className="text-xs text-muted-foreground font-black uppercase tracking-widest text-left italic">
              {original.seedName} {original.seedCompany ? `(${original.seedCompany})` : ''} | {original.cropType}
            </p>
          </div>
        </div>
        <button 
          onClick={generatePDF}
          className="flex items-center gap-3 bg-blue-600 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:brightness-110 shadow-lg transition-all active:scale-95"
        >
          <Download size={18} />
          Export PDF
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { label: 'Total Paid', value: latest.totalPaidAmount, color: 'text-green-500', barColor: 'border-l-green-500' },
      ...
      <div className="space-y-4">
        <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] px-1 text-center py-4 opacity-50">Payment History</h3>
            ...
            return (
              ...
              >
                <div className={`absolute left-0 mt-1.5 h-3 w-3 rounded-full ring-4 ring-background z-10 transition-transform group-hover:scale-125 ${
                  isOriginal ? 'bg-muted-foreground' : 'bg-blue-500 shadow-md'
                }`} />
                
                <div className="ml-6 flex-1 text-left">
                  <div className="bg-card rounded-[2.5rem] border border-border shadow-sm overflow-hidden text-left hover:border-blue-500/30 transition-all">
                    <div className={`px-6 py-3.5 flex justify-between items-center ${
                      isOriginal ? 'bg-muted/30 text-muted-foreground' : 'bg-blue-500/10 text-blue-500 font-black uppercase tracking-widest'
                    }`}>
                      ...
                      {isOriginal ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                          <div className="bg-card-secondary/50 p-4 rounded-[1.5rem] border border-border/50 text-left">
                            <p className="text-[8px] font-black text-muted-foreground uppercase mb-1 tracking-widest text-left">Seed Name / Company / Crop</p>
                            <p className="text-sm font-black text-foreground text-left">{entry.seedName} {entry.seedCompany ? `(${entry.seedCompany})` : ''} ({entry.cropType})</p>
                          </div>
                          <div className="bg-card-secondary/50 p-4 rounded-[1.5rem] border border-border/50 text-left">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
                          <div className="bg-blue-500/10 p-4 rounded-[1.5rem] border border-blue-500/20 text-left">
                            <p className="text-[8px] font-black text-blue-500 uppercase mb-1 tracking-widest text-left">Paid Now</p>
                            <p className="text-lg font-black text-blue-500 font-mono italic text-left">₹{entry.paidAmountNow.toLocaleString()}</p>
                          </div>
                          <div className="bg-card-secondary/50 p-4 rounded-[1.5rem] border border-border/50 text-left">
