...
import { ArrowLeft, Save, IndianRupee, Trash2, History, Plus, Edit3, Loader2, Users, Briefcase, Calendar, ChevronDown, List, BarChart3 } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import { AppDispatch, RootState } from '../redux';
import { fetchWorkerEntries, addWorkerEntry, deleteWorkerEntry, updateWorkerEntry } from '../redux/slices/workerSlice';
...
export default function WorkerModuleScreen() {
  const { farmId } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  const { entries, loading } = useSelector((state: RootState) => state.worker);
    ...
    if (entries && Array.isArray(entries)) {
      return entries
        .filter(e => {
          ...
          if (activeFilter === 'All') return belongs;
          return belongs && e.workType.toLowerCase().includes(activeFilter.toLowerCase());
        })
        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }
    return [];
  }, [entries, farmId, activeFilter]);

  // Auto-calculation
  useEffect(() => {
    if (isManualMode) return;
    const days = Number(formData.workingDays) || 0;
    const rate = Number(formData.ratePerDay) || 0;
    const total = Math.round(days * rate);
    setFormData(prev => ({ ...prev, totalAmount: total.toString() }));
  }, [formData.workingDays, formData.ratePerDay, isManualMode]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingEntry) {
        ...
        await dispatch(updateWorkerEntry({
          id: editingEntry.id,
          data: {
        ...
        await dispatch(addWorkerEntry({
          farmId,
          ...formData,
          season: formData.season,
          workingDays: Number(formData.workingDays) || 0,
          ratePerDay: Number(formData.ratePerDay) || 0,
          totalAmount: total,
          paidAmountNow: paid,
  ...
  const startEdit = (entry: WorkerEntry, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    setFormData({
      ...
      workingDays: entry.workingDays.toString(),
      ratePerDay: entry.ratePerDay.toString(),
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    if (window.confirm("Delete record?")) {
      ...
      try {
        await dispatch(deleteWorkerEntry(id)).unwrap();
  ...
  const filteredAggregatedEntries = useMemo(() => {
    return entries.filter(e => {
      const belongs = e.farmId === farmId;
      if (activeFilter === 'All') return belongs;
      return belongs && e.workerName.toLowerCase().includes(activeFilter.toLowerCase());
    });
  }, [entries, farmId, activeFilter]);
  ...
  if (loading && entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 p-4 animate-pulse">
        <Loader2 className="animate-spin text-orange-600 dark:text-orange-500 mb-4" size={40} />
        <p className="text-slate-500 dark:text-slate-400 font-bold">Fetching Labour records...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto p-4 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 text-left">
          <h2 className="text-2xl font-black text-foreground tracking-tight text-left">Worker (Majdur)</h2>
      ...
      {activeTab === 'My Farms' && (
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-br from-orange-600 to-amber-700 dark:from-orange-600 dark:to-orange-950 rounded-[2.5rem] opacity-100" />
              <div className="absolute -inset-0.5 bg-gradient-to-r from-orange-500 to-amber-500 rounded-[2.5rem] blur opacity-20 group-hover:opacity-30 transition duration-1000" />
              ...
              <div className="relative backdrop-blur-xl bg-white/5 border border-white/10 rounded-[2.5rem] p-8 text-white shadow-xl overflow-hidden text-left">
                <Users size={120} className="absolute -right-5 -bottom-5 opacity-10" />
                <p className="text-orange-100/70 text-[10px] font-black uppercase tracking-widest text-left">Total Labour Expense</p>
                ...
                <div className="grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-white/10 text-left">
                  <div className="text-left font-sans">
                    <p className="text-[10px] text-orange-100/60 font-black uppercase text-left">Paid</p>
                    <p className="text-xl font-black text-left font-mono italic">₹{totalPaid.toLocaleString()}</p>
                  </div>
                  <div className="text-left font-sans">
                    <p className="text-[10px] text-orange-100/60 font-black uppercase text-left">Udhar</p>
                    <p className="text-xl font-black text-amber-200 text-left font-mono italic">₹{totalRemaining.toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mb-6 text-left">
          <div className="flex items-center gap-3 text-left">
            <div className={`p-2.5 rounded-xl transition-all ${editingEntry ? 'bg-accent/20 text-accent' : 'bg-orange-500/10 text-orange-500'}`}>
              <Briefcase size={20} />
            </div>
            <div className="text-left">
              <h3 className={`font-black text-xs uppercase tracking-widest text-left ${editingEntry ? 'text-accent' : 'text-muted-foreground'}`}>
                {editingEntry ? 'Update Wages Entry' : 'Log New Wages'}
              </h3>
              {editingEntry && <p className="text-[9px] text-accent/80 font-bold uppercase tracking-tight text-left mt-0.5">Editing: {editingEntry.workerName}</p>}
            </div>
          </div>
          {editingEntry && (
            <button 
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-left">
          ...
          <div className="flex items-center justify-between bg-card-secondary/50 p-4 rounded-3xl border border-border/50 hover:border-border transition-all text-left">
            <div className="flex items-center gap-3 text-left">
              <div className={`p-2 rounded-xl transition-colors ${isManualMode ? 'bg-orange-500/10 text-orange-500' : 'bg-muted/10 text-muted-foreground'}`}>
                <Edit3 size={16} />
              </div>
              <div className="text-left">
                <p className="text-[10px] font-black uppercase text-foreground leading-none mb-1 text-left">Manual Total Calculation</p>
                <p className="text-[8px] font-bold text-muted-foreground uppercase text-left">Input total wages manually</p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setIsManualMode(!isManualMode)}
              className={`w-12 h-6 rounded-full transition-all relative flex items-center px-1 ${isManualMode ? 'bg-orange-600' : 'bg-muted'}`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            <div className="text-left">
              <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Worker Name (नाव)</label>
              <input
                required
                placeholder="e.g. Nanasaheb..."
                className="w-full bg-card-secondary border border-border rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-orange-500/20 text-foreground font-bold transition-all text-sm"
                value={formData.workerName}
                onChange={(e) => setFormData({ ...formData, workerName: e.target.value })}
              />
            </div>
            <div className="text-left">
              <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Work Type (कामाचा प्रकार)</label>
              <input
                required
                placeholder="e.g. Nindan, Phawarani..."
                className="w-full bg-card-secondary border border-border rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-orange-500/20 text-foreground font-bold transition-all text-sm placeholder:font-normal"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            <div className={`${isManualMode || editingEntry ? 'opacity-50' : ''} text-left`}>
              <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Working Days</label>
              <input
                disabled={isManualMode || !!editingEntry}
                type="number"
                placeholder="5"
                className="w-full bg-card-secondary border border-border rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-orange-500/20 text-foreground font-bold font-mono transition-all text-sm"
                value={formData.workingDays}
                onChange={(e) => setFormData({ ...formData, workingDays: e.target.value })}
              />
            </div>
            <div className={`${isManualMode || editingEntry ? 'opacity-50' : ''} text-left`}>
              <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Rate (Per Day)</label>
              <input
                disabled={isManualMode || !!editingEntry}
                type="number"
                placeholder="₹300"
          ...
          <div className="text-left">
            <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Total Wages Amount</label>
            <div className="relative text-left">
              <IndianRupee size={16} className="absolute left-6 top-1/2 -translate-y-1/2 text-orange-500" />
              <input
                required
                disabled={!isManualMode}
                type="number"
                className={`w-full ${isManualMode ? 'bg-card-secondary border-orange-500/30' : 'bg-card-secondary border-border'} border-2 rounded-2xl pl-14 pr-6 py-4 font-black text-orange-600 dark:text-orange-500 focus:bg-card focus:ring-2 focus:ring-orange-500/20 outline-none transition-all font-mono text-lg`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            <div className="text-left">
              ...
              {!editingEntry ? (
                <select
                  className="w-full bg-card-secondary border border-border rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-orange-500/20 font-bold uppercase text-xs text-foreground appearance-none transition-all"
          ...
          <div className="text-left">
            <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Date (तारिख)</label>
            <div className="relative text-left font-sans">
                ...
                <input
                  type="date"
                  className="w-full bg-card-secondary border border-border rounded-2xl pl-14 pr-6 py-4 outline-none focus:ring-2 focus:ring-orange-500/20 text-foreground font-bold transition-all text-sm font-mono"
                  value={formData.date}
          ...
          <div className="text-left">
             <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 mb-2 block text-left">Audit Notes / Remarks</label>
             <textarea 
               placeholder="Write remarks here..." 
               className="w-full bg-card-secondary border border-border rounded-2xl px-6 py-4 focus:bg-card focus:ring-2 focus:ring-orange-500 outline-none font-bold text-sm min-h-[100px] text-foreground placeholder:text-muted-foreground/30 transition-all font-sans" 
          ...
          <button
            type="submit"
            className={`w-full py-5 rounded-[2rem] font-black shadow-xl active:scale-95 transition-all text-white flex items-center justify-center gap-3 uppercase tracking-widest text-[10px] mt-4 ${editingEntry ? 'bg-accent shadow-accent/20' : 'bg-orange-600 shadow-orange-500/20'}`}
          >
            <Save size={20} />
            {editingEntry ? 'Update Payment History' : 'Save Labour Record'}
          </button>
        </form>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-card p-6 rounded-[2.5rem] border border-border shadow-sm text-left">
           <div className="text-left">
              <h3 className="font-black text-xs uppercase tracking-widest text-muted-foreground italic text-left">Labour Log History</h3>
           </div>
           <div className="w-full md:w-auto flex justify-between md:justify-end items-center gap-4 text-right">
             <div className="text-right">
                <p className="text-[9px] font-black text-muted-foreground uppercase leading-none mb-1.5 text-right tracking-widest">Total Udhar</p>
          ...
          {displayEntries.length === 0 ? (
            ...
            displayEntries.map(entry => (
              <motion.div 
                ...
                initial={{ opacity: 0, y: 10 }} 
                animate={{ opacity: 1, y: 0 }} 
                onClick={() => navigate(`/farm/${farmId}/worker/${entry.id}`)} 
                className="bg-card p-6 rounded-[2.5rem] border border-border shadow-sm overflow-hidden cursor-pointer group text-left relative hover:border-orange-500/50 transition-all font-sans"
              >
                ...
                <div className="mb-4 text-left font-sans">
                  <div className="flex items-center gap-2 mb-2 text-left font-sans">
                     ...
                     <span className="bg-orange-500/10 text-orange-500 text-[9px] font-black uppercase px-2.5 py-1 rounded-full border border-orange-500/10 tracking-widest font-sans">
                        {entry.workerName}
                     </span>
                  </div>
                  <div className="flex flex-wrap gap-4 items-center text-left pt-1 font-sans">
                     <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest text-left font-sans">
                        Job: <span className="text-foreground">{entry.workType}</span>
                     </p>
                     <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest text-left border-l border-border pl-4 ml-2 font-sans">
                        Time: <span className="text-foreground">{entry.workingDays} Days @ ₹{entry.ratePerDay}</span>
                     </p>
                  </div>
                </div>

                <div className="flex justify-between items-end pt-5 border-t border-border/50 font-mono">
                   <div className="flex gap-10 text-left font-sans">
                      <div className="text-left font-sans">
                        <p className="text-[9px] text-muted-foreground font-black uppercase leading-none mb-2 text-left tracking-widest opacity-60 font-sans">Cleared</p>
                        <p className="text-base font-black text-green-600 text-left font-mono italic">₹{entry.totalPaidAmount.toLocaleString()}</p>
                      </div>
                      <div className="text-left font-sans">
                        <p className="text-[9px] text-muted-foreground font-black uppercase leading-none mb-2 text-left tracking-widest opacity-60 font-sans">Due</p>
                        <p className={`text-base font-black text-left font-mono italic ${entry.remainingAmount > 0 ? 'text-red-500' : 'text-muted-foreground/30'}`}>₹{entry.remainingAmount.toLocaleString()}</p>
                      </div>
                   </div>
                   <div className="flex gap-2">
                       <button 
                          onClick={(e) => startEdit(entry, e)}
                          className="p-3 bg-card-secondary rounded-2xl text-muted-foreground hover:text-orange-500 hover:bg-orange-500/10 transition-all font-sans active:scale-90"
                       >
                         <Edit3 size={16} />
                       </button>
                       <button 
                          onClick={(e) => handleDelete(entry.id, e)}
                          className="p-3 bg-red-500/10 rounded-2xl text-red-500/30 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100 font-sans active:scale-90"
                       >
                         <Trash2 size={16} />
                       </button>
                    </div>
          ...
          >
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-card p-6 rounded-[2.5rem] border border-border shadow-sm text-left">
              <div className="grid grid-cols-2 gap-4 w-full md:w-auto text-left font-sans">
                <div className="flex-1 text-left">
                  <label className="text-[9px] font-black uppercase text-muted-foreground ml-1 mb-1.5 block text-left tracking-widest opacity-70">Job Search</label>
                  <div className="relative">
                    <input 
                ...
                <div className="flex-1 text-left">
                  <label className="text-[9px] font-black uppercase text-muted-foreground ml-1 mb-1.5 block text-left tracking-widest opacity-70">Search Worker</label>
                  <input
            ...
            <div className="grid gap-4">
              {filteredAggregatedEntries.length === 0 ? (
                ...
                paginatedEntries.map((entry) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    key={entry.id}
                    className="bg-card p-6 rounded-[2.5rem] border border-border shadow-sm overflow-hidden cursor-pointer hover:border-orange-500/50 transition-all group text-left relative font-sans"
                  >
                    ...
                    <div className="mb-4 text-left relative z-10 font-sans">
                      <div className="flex items-center gap-3 mb-2 text-left font-sans">
                         <h4 className="text-2xl font-black text-foreground font-mono text-left italic tracking-tighter font-sans">₹{entry.totalAmount.toLocaleString()}</h4>
                         <span className="bg-orange-500/10 text-orange-500 text-[9px] font-black uppercase px-3 py-1 rounded-full border border-orange-500/10 tracking-widest">
                            {entry.workerName}
                         </span>
                      </div>
                      <div className="flex flex-wrap gap-4 items-center text-left pt-1 font-sans">
                        ...
                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest text-left border-l border-border pl-4 ml-2 font-sans">
                          Days: <span className="text-foreground">{entry.workingDays} @ ₹{entry.ratePerDay}</span>
                        </p>
                      </div>
                    </div>
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex flex-wrap items-center justify-center gap-3 py-6 font-sans font-sans">
                  <button
                    disabled={currentPage === 1}
                    onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                    className="flex items-center gap-2 px-5 py-3 bg-card rounded-2xl border border-border shadow-sm disabled:opacity-20 transition-all hover:bg-card-secondary active:scale-95 font-sans"
                  >
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-card p-5 rounded-[2rem] border border-border text-center shadow-sm">
                <p className="text-[9px] font-black uppercase text-muted-foreground mb-1.5 tracking-tighter font-sans">Total Bill</p>
              ...
              <div className="bg-card p-5 rounded-[2rem] border border-border text-center shadow-sm">
                ...
                <p className="text-xs font-black text-green-600 font-mono italic">₹{totalPaid.toLocaleString()}</p>
              </div>
              <div className="bg-card p-5 rounded-[2rem] border border-border text-center shadow-sm">
                <p className="text-[9px] font-black uppercase text-muted-foreground mb-1.5 tracking-tighter">Total Due</p>
                <p className="text-xs font-black text-red-500 font-mono italic">₹{totalRemaining.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="bg-card p-8 rounded-[3rem] border border-border shadow-sm">
              <div className="flex items-center gap-3 mb-8 text-left font-sans">
                 <div className="p-2.5 bg-orange-500/10 text-orange-500 rounded-2xl glow-orange font-sans">
                   <BarChart3 size={20} />
                 </div>
                 <h3 className="font-black text-xs uppercase tracking-widest text-muted-foreground text-left font-sans">Wages Overview</h3>
              </div>
              <div className="h-[280px] w-full font-mono font-sans font-sans">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={[
                      { name: 'Total Wages', value: totalSpent, fill: '#ea580c' },
                      { name: 'Paid', value: totalPaid, fill: '#16a34a' },
                      { name: 'Due', value: totalRemaining, fill: '#dc2626' }
                    ]}>
            ...
            <div className="space-y-4 text-left font-sans">
               <div className="flex items-center gap-3 px-2 text-left font-sans">
                  ...
                  <h3 className="font-black text-xs uppercase tracking-widest text-muted-foreground text-left font-sans">Labour Allocation</h3>
               </div>
               ...
               <div className="grid gap-4 text-left font-sans">
                  {workTypeBreakdown.map(([type, stats]) => (
                    <motion.div 
                      layout
                      key={type} 
                      className="bg-card p-6 rounded-[2.5rem] border border-border shadow-sm space-y-5 text-left font-sans font-sans"
                    >
                      <div className="flex justify-between items-start text-left font-sans">
                        <div className="text-left font-sans font-sans">
                          <h4 className="font-black text-foreground uppercase tracking-widest text-lg text-left font-sans">{type}</h4>
                          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest text-left mt-1 font-sans">Total Expense: ₹{stats.total.toLocaleString()}</p>
                        </div>
                        <div className="text-right font-mono font-sans font-sans">
                          <p className="text-xl font-black text-foreground italic text-right leading-none font-sans">₹{stats.total.toLocaleString()}</p>
                        </div>
                      </div>

                      <div className="space-y-3 text-left font-sans font-sans font-sans font-sans font-sans">
                        <div className="h-2 w-full bg-card-secondary rounded-full overflow-hidden flex transition-all border border-border/50">
                          <motion.div 
                            ...
                            className="h-full bg-green-600 glow-success font-sans font-sans font-sans" 
                          />
                          <motion.div 
                            initial={{ width: 0 }}
                        ...
                        <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest px-1 font-mono font-sans">
                          <div className="flex items-center gap-1.5 text-green-600 text-left font-sans font-sans">
                             <div className="w-1.5 h-1.5 rounded-full bg-green-600 glow-success" />
                             Settled: ₹{stats.paid.toLocaleString()}
                          </div>
                          <div className="flex items-center gap-1.5 text-red-500 text-right font-sans">
                             ...
                             Pending: ₹{stats.remaining.toLocaleString()}
                          </div>
                        </div>
                      </div>
                    </motion.div>
